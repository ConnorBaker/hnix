...
module Nix.Value.Core.Value
  ( -- * NValueF: Base functor
    NValueF(..)
  , sequenceNValueF
  , bindNValueF
  , liftNValueF
...
import           Lens.Family2                   ( LensLike', Lens' )
...
import           Nix.Core.Expr.Types            ( Params(..)
                                                , PositionSet
                                                , VarName
                                                , NAtom(..)
                                                )
import           Nix.Types.Path                 ( Path )
import           Nix.Core.Utils                 ( whenTrue )
import           Nix.Value.Core.String          ( NixString
                                                , mkNixStringWithoutContext
                                                , ignoreContext
                                                , hasContext
                                                )
import           Nix.Value.Core.Thunk           ( MonadThunk(..) )
...
data NValueF p m r
    ...
    | NVPathF Path
    | NVListF (NixList r)
      -- ^ Nix lists use NixList abstraction for O(1) length and indexing.
...
-- | Haskell NixList to the Nix list (O(1) length and indexing),
pattern NVList' :: NVConstraint w => NixList a -> NValue' t w m a
...
iterNValue
  ...
  -> NValue t f m
  -> r
iterNValue k f = fix ((iter f .) . fmap . k)
...
key k = nValue . traverse . _NVSetF . _2 . hashAt k
  where
    -- hashAt :: (Eq k, Hashable k) => k -> Lens' (AttrSet v) (Maybe v)
