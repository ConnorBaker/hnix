...
parseStoreMode :: String -> Either String StoreMode
parseStoreMode = \case
  "remote"  -> Right StoreRemote
  "overlay" -> Right StoreOverlay
  other     -> Left $ "Unsupported store mode '" <> other <> "'. Use 'remote' or 'overlay'."
  ...
  Options . fromMaybe Informational <$>
    ...
    <*> fmap fromString
        (strOption
          ...
          <> value "/nix/store"
          <> showDefault
          <> help "Nix store directory prefix"
          )
        )
    <*> option
        (eitherReader parseStoreMode)
        (  long "store-mode"
        <> metavar "remote|overlay"
        <> value StoreRemote
        <> showDefault
        <> help "Store backend: remote (nix-daemon) or overlay (in-memory, read-through)"
        )
    <*> (not <$> switch
        (  long "no-store-read-through"
        <> help "Disable read-through to system store in overlay mode (fully isolated)"
        ))
